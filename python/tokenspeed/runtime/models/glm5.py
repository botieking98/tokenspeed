...
class FusedGateUpMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        quant_inter, pertoken_inter = torch_npu.npu_dynamic_quant(inter, dst_type=torch.int8)
        if pertoken_inter.dim() == 2:
            quant_inter = quant_inter.squeeze(1)
            pertoken_inter = pertoken_inter.squeeze(1)
        return torch_npu.npu_quant_matmul(
...
class GlmMoeDsaAttention(nn.Module):
    ...
    def _forward_decode(
        ...
        # Apply RMSNorm to kv_a (was previously fused in npu_kv_rmsnorm_rope_cache)
        kv_a = latent_cache[..., :self.kv_lora_rank]
        k_pe = latent_cache[..., self.kv_lora_rank:]
